...
// 測試用 實際 production 請 存進資料庫
const tokenHash = {}
app.get('/setAccessToken', (req, res) => {
  console.log(req.query)
  tokenHash[req.query.pageID] = req.query.token
  res.status(200).send('EVENT_RECEIVED');
})

const request = require('request');
function callSendAPI(sender_psid, response, pageID) {
  console.log(tokenHash[pageID])
  ...
  request({
    ...
    "qs": { "access_token": tokenHash[pageID] },
    "method": "POST",
    "json": request_body
  }, (err, res, body) => {
    if (!err) {
...
function handleMessage(sender_psid, received_message, pageID) {
  ...
  if (received_message.text) {    
    ...
    response = {
      "text": `抹綠收到: "${received_message.text}"`
    }
  }  
